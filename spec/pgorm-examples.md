...
但还有一些“能力已经实现、却没有可运行范例”的部分，用户（尤其是新用户）会很难知道：
...
#### 2) `insert_many`（UNNEST bulk insert）
...
#### 5) `monitoring`（InstrumentedClient + Hook）
...
#### 6) `jsonb`（Json<T> / serde_json::Value）
...
（这个例子也可以只做成 docs 里的“语义说明”，不一定要 runnable。）
